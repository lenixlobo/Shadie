...
int main()
{
	...
	dragon_normal = new float[3*elements.size()];
	int vertexPtr = 0;
	...
	std::cout << "\nvertices: " <<vertices.size()<< "elements: " <<elements.size()<< "normals: "<<normals.size()<<std::endl;
	...
	glBufferData(GL_ARRAY_BUFFER, 3 * elements.size() * sizeof(float), dragon_normal, GL_STATIC_DRAW);
